Shader "RzShader/Rz_SlashStretch_Mesh" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                float node_8758 = 1.0;
                float node_6744 = 0.0;
                float2 node_6376 = float2(((node_6744 + ( (i.uv0.r - _Y_Stretch) * (node_8758 - node_6744) ) / (node_8758 - _Y_Stretch))+_X_SlashOffset),i.uv0.g);
                float4 node_1324 = tex2D(_MainTex,TRANSFORM_TEX(node_6376, _MainTex));
                float3 emissive = (((node_1324.r*_HighlightStrength)+(_Color.rgb*node_1324.g*_Emission))*_Z_Emission*i.vertexColor.rgb);
                float3 finalColor = emissive;
                float4 node_1050 = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float2 UV = i.uv0;
                float2 node_4830 = UV;
                float4 node_5871 = _Time;
                float2 node_7932 = (node_4830+node_5871.g*float2(-0.5,0.05));
                float4 node_5406 = tex2D(_FlowMap,TRANSFORM_TEX(node_7932, _FlowMap));
                float2 node_6917 = (node_4830+node_5871.g*float2(-0.3,-0.1));
                ...
                return fixed4(finalColor,(i.vertexColor.a*(node_1324.g*node_1050.b*saturate(step(0.5,(node_319.a+(_W_Dissolve-0.5)))))));
            }
            ENDCG
        }
    }
